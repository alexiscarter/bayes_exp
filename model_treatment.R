...
Xc<-model.matrix(~ treatment , data= trait_as_ir)

## Create a covariable matrix for the survival
Xb<-model.matrix(~ treatment, data= trait_as_ir)
...
summary(tt)$adj.r.squared #0.2714946

## sketch the model fit
MyData <- expand.grid(Treatment = levels(trait_as_ir$treatment))

X<-model.matrix(~ Treatment , data= MyData)
